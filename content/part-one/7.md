...
MBI image suites---including visible, ultraviolet-induced visible fluorescence (UVF), ultraviolet reflectance (UVR), IRR, and visible-induced infrared luminescence (VIL) images---of each portrait and paint-out board were captured using a modified Nikon D610 DSLR camera with the UV/IR filters removed and a Jenoptik 60 millimeter UV-VIS-IR Apo Macro lens (see [fig. 7.2](#fig_7_2) for lighting and filter specifications). False-color ultraviolet reflectance (FCUV) and false-color infrared reflectance (FCIR) images were generated by combining reflectance captures via channel substitution in Adobe Photoshop®.[^5] A Spectralon 99-percent reflectance standard from Labsphere,[^6] an X-Rite ColorChecker Passport, and an unbound indigo pigment sample[^7] were included in all captures ([fig. 7.1](#fig_7_1)). An unbound Egyptian blue pigment sample was included in all VIL captures. MBI suites of the paint-out boards included dry samples of the binding media and pigments used on each board ([fig. 7.3](#fig_7_3)).
{{< q-figure id="fig_7_3" >}}
